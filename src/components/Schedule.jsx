...
const Schedule = ({ scheduleUser, allowEdit, header }) => {
  ...
  async function getPageEvents(fetchInfo, successCallback) {
    ...
    if (isCached) {
      isCached =
        fetchInfo.start.toDateString() ==
          calendarState.current.cachedRange.start.toDateString() ||
        fetchInfo.end.toDateString() ==
          calendarState.current.cachedRange.end.toDateString();
    }
    if (!isCached || calendarState.current.events.length == 0) {
      ...
      calendarState.current.cachedRange = {
    ...
    if (allowEdit) {
      ...
      events = events.filter(function (event) {
        return (
          calendarState.current.shiftsRemoved.find((id) => id == event.id) ==
          undefined
        );
      });
      ...
      events = events.filter(
        (event) =>
          event.startTime > fetchInfo.start && event.endTime < fetchInfo.end
      );
    }
  ...
  function getEditedEventObject(id) {
    ...
    return calendarState.current.shiftsEdited[
      calendarState.current.shiftsEdited.push(event) - 1
    ];
  }
  ...
  async function markShiftCompleted() {
    ...
    calendarState.current.events[
      calendarState.current.events.indexOf(activeEvent)
    ] = await getShift(userToken, activeEvent.id); //ensure sync in case api fails
  }
  //Pass pending edits to API, wipe state
  async function saveEdits() {
    await saveScheduleEdits(
      userToken,
      scheduleUser,
      calendarState.current.shiftsEdited,
      calendarState.current.shiftsRemoved,
      calendarState.current.shiftsAdded
    );
  ...
  function saveEventChanges() {
    ...
    var times = [
      ...
      activeEvent.startTime.getSeconds(),
    ];
    editEvent.startTime = new Date(
      stringTok[0],
      stringTok[1],
      stringTok[2],
      times[0],
    ...
    times = [
      activeEvent.endTime.getHours(),
      activeEvent.endTime.getMinutes(),
      activeEvent.endTime.getSeconds(),
    ];
    editEvent.endTime = new Date(
      stringTok[0],
      stringTok[1],
      stringTok[2],
      times[0],
  ...
  function promptDeleteEvent() {
    if (confirm("Are you sure you want to delete this shift?")) {
      saveCalendarState();
      var pos = calendarState.current.shiftsEdited.findIndex(
        (i) => i.id == activeEvent.id
      );
      if (pos != -1) calendarState.current.shiftsEdited.splice(pos, 1);
      pos = calendarState.current.shiftsAdded.findIndex(
        (i) => i.id == activeEvent.id
      );
      if (pos != -1) calendarState.current.shiftsAdded.splice(pos, 1);
      else calendarState.current.shiftsRemoved.push(activeEvent.id);
      setShowViewPanel(false);
    }
  }
  //Save the calendar scroll position and range for the re-render
  function saveCalendarState() {
    calendarState.current.scroll = document.querySelector(
      ".fc-scroller-liquid-absolute"
    ).scrollTop;
    calendarState.current.visibleRange = calendarRef.current
      .getApi()
      .currentData.dateProfile.activeRange.start.toLocaleDateString([], {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
      });
  ...
  const calcTimeEstimateString =
    String(Math.floor(calendarState.current.scroll / 80)).padStart(2, "0") +
    ":" +
    String(Math.floor((calendarState.current.scroll % 80) / 1.333333)).padStart(
      2,
      "0"
    );
  const options = {
    ...
    slotLaneDidMount: function (arg) {
      if (arg.el.parentNode.childNodes[0].childNodes.length > 0) {
        const timeHeader =
          arg.el.parentNode.childNodes[0].childNodes[0].childNodes[0];
        timeHeader.style =
          "font-size:16pt; position:absolute;width:0px; text-wrap: nowrap; margin-top:-8px";
      }
    },
    ...
    scrollTime: calendarState.current.scroll
      ? calcTimeEstimateString
      : pseudoNow.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
          hour12: false,
        }),
    ...
    eventClick: async (eventClickInfo) => {
      const e = await getEventObject(
        eventClickInfo.event.extendedProps.eventId
      );
      saveCalendarState();
      setActiveEvent(e);
  ...
  if (allowEdit) {
    ...
    options.eventResize = function (eventResizeInfo) {
      const eventObj = getEditedEventObject(
        eventResizeInfo.event.extendedProps.eventId
      );
    ...
    options.eventDrop = function (eventDropInfo) {
      const eventObj = getEditedEventObject(
        eventDropInfo.event.extendedProps.eventId
      );
    ...
    <section className="relative w-full">
      <GlobalStyleWrapper>
        {showViewPanel && !allowEdit && (
          <div
            className="fixed inset-0 flex text-center items-center justify-center bg-black bg-opacity-50 p-8 z-10"
            onClick={() => {
              saveCalendarState();
              setShowViewPanel(false);
            }}
          >
            <div className="inline-block m-auto align-top">
              <div
                className="m-3 h-full border-2 border-solid rounded border-black text-neutral-400 bg-secondary bg-gradient-to-tr from-secondary to-neutral-600"
                onClick={(event) => {
              ...
              >
                ...
                <div className="m-2">
                  ...
                  <input
                    ...
                    className="ml-2 mr-3"
                    disabled
                    defaultChecked={activeEvent.completed}
                  ></input>
                  {!activeEvent.completed && (
                    <button
                      className="bg-forth align-middle hover:bg-fifth text-white px-1 rounded"
                      onClick={markShiftCompleted}
                    >
                      Mark as Completed
                    </button>
                  )}
            ...
            <div className="inline-block m-auto align-top">
              ...
              >
                ...
                <h2 className="text-2xl ml-5 mr-5 rounded p-0.5 bg-forth text-white font-bold">
                  <input
                    ref={editorName}
                    className="transparent-input bg-forth text-center border-2 border-solid rounded border-black"
                ...
                <input
                  type="date"
                  ref={editorDate}
                  className="transparent-input mt-2 text-neutral-200"
                  defaultValue={getISONoTimeZone(activeEvent.startTime)}
                ></input>
                <div className="mt-2 text-neutral-200">
                  {activeEvent.startTime.toLocaleTimeString([], {
                    hour: "2-digit",
                ...
                <textarea
                  ref={editorDesc}
                  className="min-w-80 min-h-1/2 h-min ml-5 mr-5 mt-1 h-full border-2 border-solid rounded border-neutral-800 bg-neutral-700"
                  defaultValue={activeEvent.desc}
                ></textarea>
                <div className="m-2">
                  Completed:
                  <input
                    ref={editorComplete}
                    type="checkbox"
                    className="transparent-input ml-2 mr-3"
                    defaultChecked={activeEvent.completed}
                  ></input>
                </div>
                <button
                  className="mb-3 text-lg bg-forth align-middle hover:bg-fifth text-white px-1 rounded"
                  onClick={saveEventChanges}
                >
                  Save Changes
                </button>
                <button
                  className="ml-6 mb-3 text-lg bg-forth align-middle hover:bg-fifth text-white px-1 rounded"
        ...
        <div className="inline-block h-full m-auto w-full align-top text-center">
          <div className="m-3 h-full border-2 border-solid rounded border-black text-neutral-400 bg-secondary bg-gradient-to-tr from-secondary to-neutral-600">
            {header && (
              <h3 className="text-2xl w-full rounded p-0.5 bg-forth text-white font-bold">
                {header}
              </h3>
            )}
            <hr className="border-1 border-black"></hr>
            <TableStyleWrapper>{renderedCalendar.current}</TableStyleWrapper>
            {allowEdit && (
              <div>
                <span
                  ref={editorDraggable}
                  className="m-3 text-xl bg-forth align-middle hover:bg-fifth hover:cursor-pointer text-white px-1 rounded"
                >
                  Add Shift (Drag)
                </span>
                <button
                  className="m-3 text-xl bg-forth align-middle hover:bg-fifth text-white px-1 rounded"
                  onClick={saveEdits}
                >
                  Save Changes
                </button>
